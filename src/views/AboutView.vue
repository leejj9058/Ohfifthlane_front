            ...
            <div v-if="reservationDateTimeModal || showRPZDetailModal" class="bottom-sheet">
                ...
                <div v-if="showRPZDetailModal" class="modal-content-rpz" @click="moveReservation(selectedRPZ.id)">
                    ...
                    <div class="timebar-container">
                        <div class="timebar">
                            <div v-for="time in availableTimes" :key="time" class="timebar-block"
                                :class="{ available: isAvailable(time), reserved: !isAvailable(time) }"
                                @click="handleTimeClick(time)">
                            </div>
                        </div>
                        <!-- 시간 레이블 두 줄로 표시 -->
                        <div class="time-labels">
                            <span v-for="label in timeLabels.slice(0, 5)" :key="label.time"
                                :style="{ left: label.position + '%' }">{{ label.time }}</span>
                        </div>
                        <div class="time-labels">
                            <span v-for="label in timeLabels.slice(5, 10)" :key="label.time"
                                :style="{ left: label.position + '%' }">{{ label.time }}</span>
                        </div>
                    </div>
...
const reservedTimes = ref(["10:00", "10:10", "10:20", "10:30", "14:10", "16:30"]);
...
const handleTimeClick = (time) => {
  if (isAvailable(time)) {
    console.log(`${time} 시간 예약 가능`);
  } else {
    console.log(`${time} 시간은 예약이 이미 차 있습니다.`);
  }
};

const timeLabels = ref([
  { time: '0시', position: 0 },
  { time: '3시', position: 12.5 },
  { time: '6시', position: 25 },
  { time: '9시', position: 37.5 },
  { time: '12시', position: 50 },
  { time: '15시', position: 62.5 },
  { time: '18시', position: 75 },
  { time: '21시', position: 87.5 },
  { time: '24시', position: 100 }
]);
...
const chargingStationInfo = async () => {
    ...
    try {
        const response = await axios.get('/api/getChargingStation');
...
.timebar-block.available {
    background-color: #297eda;
}

.timebar-block.reserved {
